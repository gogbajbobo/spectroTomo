...
# %%
im_alpha_NaCl = im_alpha[:100,:,:].flatten()

thresh_NaCl = 0.15
mask_1 = im_poly_gauss_filtered > thresh_NaCl

mask_NaCl = mask_1[:100,:,:].flatten()

plt.hist(im_alpha_NaCl[mask_NaCl == 1], bins=128)
plt.xlabel(r'µ @ MoK$_\alpha$', fontsize=16)
plt.ylabel('Number of voxels', fontsize=16)
plt.axvline(x=1.67, c='red')
plt.text(1.7, 8700, r'µ$_{NaCl}$ @ MoK$_\alpha$', rotation='vertical', fontsize=14, c='red')
plt.grid()
plt.show()
